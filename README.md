...
- 将零散课程资料整理为结构化复习提纲，并把知识内容、公式、方法和适用条件直接写入提纲。
- 优先使用 MarkItDown 将 PPT、Word、PDF 等资料转换为 Markdown。
- MarkItDown 不可用时，会询问用户是安装后转换，还是直接读取源文件。
- 转换结果乱码、缺页或内容不完整时，会回退读取原始文件。
- 自动提炼并总结高频考点、概念关系、公式、定义、推导/方法步骤、易错点和题型规律。
- 使用 Markdown/LaTeX 数学格式优化公式显示，完善上下标、分式、求和、积分、矩阵和分段函数等表达。
...
- `复习提纲.md`：章节化知识点、高亮重点、内容总结、可渲染公式/方法、适用条件、考点优先级和易错提醒。
